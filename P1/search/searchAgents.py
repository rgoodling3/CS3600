...
def cornersHeuristic(state, problem):
    ...
    bleeblah = list(state[1])
    state = state[0]
    total = 0

    while len(bleeblah) > 0 : 
        distances = list(map(lambda x: util.manhattanDistance(x, state), bleeblah))
        nextcorner = min(distances)
        total += nextcorner
        oldcorner = bleeblah[distances.index(nextcorner)]
        bleeblah.remove(oldcorner)
        state = oldcorner
